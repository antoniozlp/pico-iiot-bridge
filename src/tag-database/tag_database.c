...
bool tag_db_save_to_flash(void)
{
    ...
    uint16_t config_slot = 0;
    
    // Copy only allocated tags to config structure
    for (uint16_t i = 0; i < s_tag_high_water_mark && config_slot < TAG_DB_MAX_PERSISTENT_TAGS; i++)
    {
        if (s_tags[i].allocated)
        {
            strncpy(config.tags[config_slot].name, s_tags[i].name, TAG_NAME_MAX_LEN - 1);
            config.tags[config_slot].name[TAG_NAME_MAX_LEN - 1] = '\0';
            config.tags[config_slot].data_type = (uint8_t)s_tags[i].data_type;
            config.tags[config_slot].enabled = 1;
            memset(config.tags[config_slot].reserved, 0, sizeof(config.tags[config_slot].reserved));
            config_slot++;
            saved_count++;
        }
    }
    ...
    LOG_INFO("Saved %d tags to flash", saved_count);
    return true;
}
